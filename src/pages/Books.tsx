...
export default function Books() {
  ...
  //     ...form,
  //     [name]:
  //       name === "published_year" || name === "available_copies"
  //         ? Number(value)
  //         : value,
  //   });
  // };
  ...
  //       alert("Book updated!");
  //     } else {
  //       const res = await axios.post("http://localhost:3000/books", form, {
  //         headers: { Authorization: `Bearer ${token}` },
  //       });
  ...
  //   setForm({ ...book, genre_id: book.genre.id });
...
const handleChange = (
  ...
  setForm({
    ...form,
    [name]:
      name === "published_year" || name === "available_copies" || name === "genre_id"
        ? Number(value)
        : value,
  });
};
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (editingId) {
      ...
      setBooks((prev) => prev.map((b) => (b.id === editingId ? res.data : b)));
      alert("Book updated!");
    } else {
